...
long double s21_pow(double base, double exp) {
    ...
    //  pow(±0, exp), where exp is negative, finite, and is an even integer or a non-integer,
    //  returns +∞ and raises FE_DIVBYZERO
    if (s21_fabs(base) <= S21_EPS && e_f && ((e_i && (int) exp % 2 == 0) || !e_i)) {
        return S21_INF;
    }

    //  pow(±0, -∞) returns +∞ and may raise FE_DIVBYZERO
    if (s21_fabs(base) <= S21_EPS && !e_f && !e_n && exp < 0) {
        return S21_INF;
    }
    ...
    if (s21_fabs(base) <= S21_EPS && exp > 0 && (!e_i || ((int) exp % 2 == 0 && e_i))) {
        return +0;
    }
